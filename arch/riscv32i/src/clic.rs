...
impl Clic {
    ...
    pub fn next_pending(&self) -> Option<u32> {
        ...
        if self.in_use_interrupts & (1 << 3) > 0 && self.registers.clicintip.msip.is_set(intpend::IntPend) {
            return Some(3);
        } else if self.in_use_interrupts & (1 << 7) > 0 && self.registers.clicintip.mtip.is_set(intpend::IntPend) {
            return Some(7);
        } else if self.in_use_interrupts & (1 << 11) > 0 && self.registers.clicintip.meip.is_set(intpend::IntPend) {
            return Some(11);
        } else if self.in_use_interrupts & (1 << 12) > 0 && self.registers.clicintip.csip.is_set(intpend::IntPend) {
            return Some(12);
        }
